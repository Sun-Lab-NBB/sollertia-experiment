...
from .mcp_instance import mcp, read_yaml, serialize, describe_dataclass, write_yaml_validated
...
def validate_system_configuration_tool() -> dict[str, Any]:
    ...
    paths = build_filesystem_paths_report(configuration=configuration)
    issues = [
    ...
    if configuration.video_tracking.dlc_project_path != Path() and paths["dlc_project"].get("ok", False):
        task_issue = check_dlc_project_task(project_path=configuration.video_tracking.dlc_project_path)
